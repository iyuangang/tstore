...
// Command line tool for installing ttmai
...
//                               --http_server http://localhost/ttmai
...
define('DIR_TTMAI', str_replace('\\', '/', realpath(DIR_APPLICATION . '../')) . '/');
...
function check_requirements() {
    ...
    if (phpversion() < '5.0') {
        ...
        $error = 'Warning: You need to use PHP5 or above for Application to work!';
    ...
    if (ini_get('session.auto_start')) {
        ...
        $error = 'Warning: Application will not work with session.auto_start enabled!';
    ...
    if (!extension_loaded('mysqli')) {
        ...
        $error = 'Warning: MySQLi extension needs to be loaded for Application to work!';
    ...
    if (!extension_loaded('gd')) {
        ...
        $error = 'Warning: GD extension needs to be loaded for Application to work!';
    ...
    if (!extension_loaded('curl')) {
        ...
        $error = 'Warning: CURL extension needs to be loaded for Application to work!';
    ...
    if (!function_exists('mcrypt_encrypt')) {
        ...
        $error = 'Warning: mCrypt extension needs to be loaded for Application to work!';
    ...
    if (!extension_loaded('zlib')) {
        ...
        $error = 'Warning: ZLIB extension needs to be loaded for Application to work!';
...
function setup_db($data) {
    ...
    $file = DIR_APPLICATION . 'tstore.sql';
...
function write_config_files($options) {
    ...
    $output .= 'define(\'DIR_APPLICATION\', \'' . DIR_TTMAI . 'catalog/\');' . "\n";

    $output .= 'define(\'DIR_SYSTEM\', \'' . DIR_TTMAI . 'system/\');' . "\n";

    $output .= 'define(\'DIR_DATABASE\', \'' . DIR_TTMAI . 'system/database/\');' . "\n";

    $output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_TTMAI . 'catalog/language/\');' . "\n";

    $output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_TTMAI . 'catalog/view/theme/\');' . "\n";

    $output .= 'define(\'DIR_CONFIG\', \'' . DIR_TTMAI . 'system/config/\');' . "\n";

    $output .= 'define(\'DIR_IMAGE\', \'' . DIR_TTMAI . 'image/\');' . "\n";

    $output .= 'define(\'DIR_CACHE\', \'' . DIR_TTMAI . 'system/storage/cache/\');' . "\n";

    $output .= 'define(\'DIR_DOWNLOAD\', \'' . DIR_TTMAI . 'system/storage/download/\');' . "\n";

    $output .= 'define(\'DIR_UPLOAD\', \'' . DIR_TTMAI . 'system/storage/upload/\');' . "\n";

    $output .= 'define(\'DIR_MODIFICATION\', \'' . DIR_TTMAI . 'system/storage/modification/\');' . "\n";

    $output .= 'define(\'DIR_LOGS\', \'' . DIR_TTMAI . 'system/storage/logs/\');' . "\n\n";
    ...
    $file = fopen(DIR_TTMAI . 'config.php', 'w');
    ...
    $output .= 'define(\'DIR_APPLICATION\', \'' . DIR_TTMAI . 'admin/\');' . "\n";

    $output .= 'define(\'DIR_SYSTEM\', \'' . DIR_TTMAI . 'system/\');' . "\n";

    $output .= 'define(\'DIR_DATABASE\', \'' . DIR_TTMAI . 'system/database/\');' . "\n";

    $output .= 'define(\'DIR_LANGUAGE\', \'' . DIR_TTMAI . 'admin/language/\');' . "\n";

    $output .= 'define(\'DIR_TEMPLATE\', \'' . DIR_TTMAI . 'admin/view/template/\');' . "\n";
    ...
    $output .= 'define(\'DIR_CATALOG\', \'' . DIR_TTMAI . 'catalog/\');' . "\n\n";
    ...
    $file = fopen(DIR_TTMAI . 'admin/config.php', 'w');
...
function dir_permissions() {
    ...
    $dirs = array(
        ...
        DIR_TTMAI . 'system/storage/download/',

        DIR_TTMAI . 'system/storage/upload/',

        DIR_TTMAI . 'system/storage/cache/',

        DIR_TTMAI . 'system/storage/logs/',

        DIR_TTMAI . 'system/storage/modification/',
...
case "install":
    ...
    try {
        ...
        define('HTTP_TTMAI', $options['http_server']);
